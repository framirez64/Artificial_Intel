...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        weights, featureFunction = self.getWeights(), self.featExtractor.getFeatures(state, action)
        transition = (reward + self.discount * self.computeValueFromQValues(nextState)) - self.getQValue(state, action)
        for feature in featureFunction:
            weights[feature] += self.alpha * transition * featureFunction[feature]
